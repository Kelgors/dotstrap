...
use crate::{
    action::{
        compact_mergeable_actions, transform_package_to_actions, SystemAction, generation::generate_shell_script,
    },
    host::HostDefinition,
...
pub fn run_generate(hostname: String, options: RunGenerateOptions) -> Result<()> {
    ...
    let all_actions: Vec<SystemAction> = build_action_diff(&next_system_actions)?;
